...
if have_labels:
    ...
    print(f"{'phase':>10} {'n':>7} {'|dF_contact|/mg':>16} {'|dF_fluid|/mg':>14}")
    for p in (0, 1, 2):
        sel = ph == p
        if sel.sum() == 0:
            continue
        ...
        print(f"{names[p]:>10} {int(sel.sum()):>7} {ec:>16.4f} {ef:>14.4f}")

    def _r2(a, b):
        ss = ((a - b) ** 2).sum()
        return 1.0 - ss / max(((b - b.mean(0)) ** 2).sum(), 1e-12)

    print(f"\nR^2 contact force (all frames, 3 comps pooled): {_r2(Fc_p, Fc_t):.3f}")
    print(f"R^2 fluid   force (all frames, 3 comps pooled): {_r2(Ff_p, Ff_t):.3f}")

    Tc_p = np.concatenate(pooled["Tc_pred"]); Tc_t = np.concatenate(pooled["Tc_true"])
    Tf_p = np.concatenate(pooled["Tf_pred"]); Tf_t = np.concatenate(pooled["Tf_true"])
    mgh = mg * 0.0524                      # torque scale: m g * half_width
    print(f"Torque MAE / (m g half_width):  contact "
          f"{np.linalg.norm(Tc_p - Tc_t, axis=1).mean() / mgh:.4f}   fluid "
          f"{np.linalg.norm(Tf_p - Tf_t, axis=1).mean() / mgh:.4f}"
          f"   (true fluid torque is tiny in MuJoCo; this mostly checks the"
          f" head stays quiet)")
